...
=head1 PACKAGE typing

 This package contains functions to handle Fortran types.
...
use utils('my_die','trim');
use expr_parse ('parse_expr', 'parse_part_as_expr');
...
%typing::default_type = (
                         ...
                         'doubleprecision' => $typing::double_precision,
                        );
...
sub new_scope
{
   ...
   if (@typing::typeofs)
   {
      $typing::typeof  = %{$typing::typeof};
      $typing::typedef = %{$typing::typedef};
   ...
   else
   {
      ...
      $typing::code{"//"} = [
                             {
                              'parms'  => [$typing::wild_type{'character'}, $typing::wild_type{'character'}],
                              'return' => $typing::wild_type{'character'}
                             }
                            ];
      my ($int, $real, $logical, $char) =
        ($typing::wild_type{'integer'}, $typing::wild_type{'real'}, $typing::wild_type{'logical'}, $typing::wild_type{'character'});
      my ($op);
      foreach $op ("+", "-", "*", "/")
      {
         $typing::code->{$op} = [
      ...
      foreach $op ("u+", "u-")
      {
         $typing::code->{$op} = [{'parms' => [$int], 'return' => $int}, {'parms' => [$real], 'return' => $real}];
      }
      foreach $op ("<", "<=", "==", "/=", ">", ">=")
      {
         $typing::code->{$op} = [
      ...
      {
         $typing::code->{$op} = [{'parms' => [$logical, $logical], 'return' => $logical}];
      }
      $typing::code->{".not."} = [{'parms' => [$logical], 'return' => $logical}];
      $typing::code->{"//"} = [{'parms' => [$char, $char], 'return' => $char}];
   }
...
sub make_type
{
   my ($base, $sub, $print, $star, $kind) = @_;
   my $type = {'base' => $base};
   $type->{'sub'}   = $sub   if $sub;
   $type->{'print'} = $print if $print;
   $type->{'star'} = 1 if $star;
   $type->{'kind'} = 1 if $kind;
   use htmling;
   return $type;
...
sub make_complex_type
{
   ...
   my_die("Complex constant must have real and/or integer parts, " .
          "but I found types $base1 and $base2")
...
sub is_default_kind
{
   ...
   if ($type->{'base'} eq "character")
   {
      ...
      return ($top eq "%const" && $rest[0] eq $typing::default_type{'integer'} && $rest[1] == $typing::default_character_kind);
   }
   else
   {
...
sub type_to_f90
{
   ...
   # Use star-notation?
   my $use_star = ($mkheader::force_star || ($type->{'star'} && !$mkheader::force_kind));
...
sub expr_to_f90
{
   ...
   if ($op eq "%") {
      ...
      my ($left, $right) = @children;
      $answer = $left . " = " . (expr_to_f90($right))[0];    # = has ultimately low precedence
   } elsif ($op eq "%do") {
      my ($child, $var, @args) = @children;
      $answer = "(" . expr_to_f90($child) . ", " . $var . " = " . join(", ", map { (expr_to_f90($_))[0] } @args) . ")";
   } elsif ($op eq "%call") {
      ...
      my_die("Unrecognized operation $op with " . (scalar @children) . " children");
   }
...
sub expr_type
{
   ...
   if ($op eq "%")
   {
      ...
      my_die("\%$elem failed: left part is not a compound type")
        unless $type->{'base'} eq "type";
      my ($typedef)  = $typing::typedef->{$type->{'sub'}};
      my ($elemtype) = $typedef->{$elem};
      my_die("\%$elem failed: left part does not include $elem")
        unless $elemtype;
      return $elemtype;
   }
   elsif ($op eq "%var")
   {
      ...
      my_die("Variable $var undefined") unless $vartype;
      return $vartype;
   }
   elsif ($op eq "%const")
   {
   ...
   elsif ($op eq "%colon")
   {
      ...
      my_die("colon notation for non-character string")
        if $stringtype->{'base'} ne "character";
      my_die("colon notation for character array")
        if $stringtype->{'dimension'};
   ...
   elsif ($op eq "%call")
   {
      ...
      else
      {
         ...
         {
            my_die("Array/function call for something that is neither")
              unless $optype->{'base'} eq "interface";
   ...
   my_die("Operation/function $op undefined") unless $opcodes;
   my (@childtypes) = ();
   my ($child);
   ...
   foreach $opcode (@$opcodes)
   {
      ...
      if (typing::subtypes_list(\@childtypes, $opcode->{'parms'}))
      {
         ...
         else
         {
            ...
            my_die("Internal error caused by new_scope")
              if $maxkind < 0;
            return {%$ret, 'sub' => $maxkind};
         }
      }
   }
   my_die("Operation/function $op defined but not for this (these) type(s)");
}
...
=head3 ($len,$kind) = parse_character_arguments($star,$args);

  Return the length and the kind for the character declaration

  INPUT VARIABLES
     $star - the declaration was like character*$args yes/no
     $args - the arguments after the character declaration
  OUTPUT VARIABLES
     $len  - the length of the character string
     $kind - the kind of characters (possibly undefined on output)
...
=cut
   ...
   # character*$args declaration: return $len
   if ($star)
   {
      if ($args =~ /^\(\s*\*\s*\)$/)
      {
   ...
   # character(kind=$args), possibly with len specified after
   if ($args =~ /^kind\s*=\s*/i)
   {
      $args = substr($args, length($&));
      ($kind, my $rest) = parse_part_as_expr($args);
      if (defined $rest)
      {
         $rest = trim($rest);
         $rest =~ s/^len\s*=\s*//i;
         $len = ($rest eq '*' ? '*' : parse_expr($rest));
      }
      return ($len, $kind);
   }

   # character(len=$args), possibly with kind specified after
   if ($args =~ /^len\s*=\s*/i)
   {
      $args = substr($args, length($&));
      my $rest;
      ...
      {
         ($len, $rest) = parse_part_as_expr($args);
      }
      ...
      if (defined $rest)
      {
         $rest = trim($rest);
         $rest =~ /^kind\s*=\s*/
           or my_die("kind= specifier needed when len= specifier is given");
         $rest = substr($rest, length($&));
         $kind = parse_expr($rest);
      }
   ...
   if (substr($args, 0, 1) eq '*')
   {
      ...
      # character(*) declaration, possibly $kind specified also
      $len  = "*";
      $rest = $args;
      $rest =~ s/^\*\s*,// or $rest = undef;
   }
   else
   {
      ...
      # character($len), possibly $kind specified also
      ($len, $rest) = parse_part_as_expr($args);
   }
